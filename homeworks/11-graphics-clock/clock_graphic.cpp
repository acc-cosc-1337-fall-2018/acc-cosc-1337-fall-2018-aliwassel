...
{
	device_context->SetPen(wxPen(wxColor(0, 0, 0), 1)); // green blue
	clock_rim->draw();
...
void ClockGraphic::draw_seconds_markers()
{
	for (int i = 0; i < 60; i++) 
	{
		...
		double seconds_angle = i / 60.0 * 360.0; // seconds angle 
...
double ClockGraphic::get_angle(double degrees) // draw seconds 
{
